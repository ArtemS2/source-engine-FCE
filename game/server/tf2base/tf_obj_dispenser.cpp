...
bool CObjectDispenser::DispenseAmmo( CTFPlayer *pPlayer )
{
	...
	// metal (Aptems1 hack, fixme if you can)
	int iMetal = pPlayer->GiveAmmo( min( m_iAmmoMetal.Get(), DISPENSER_DROP_METAL ), TF_AMMO_METAL );
	m_iAmmoMetal -= iMetal;
...
void CObjectDispenser::RefillThink( void )
{
	...
	{      
		// (Aptems1 hack, fixme if you can)
		m_iAmmoMetal = min(static_cast<int>(m_iAmmoMetal.Get() + DISPENSER_MAX_METAL_AMMO * 0.1), DISPENSER_MAX_METAL_AMMO );
